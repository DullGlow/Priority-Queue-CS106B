...
void printReplInstructions() {
	...
	cout << "   toString:       Shows content of the priority queue." << endl;
	cout << "   quit:           Quits the interpret and returns to the menu." << endl;
    cout << endl << "Fibonacci-only commands:" << endl;
    cout << endl;
    cout << "   decreaseKey:    Finds node with similar properties and decreases key." << endl;
	cout << endl;
		...
		while (true) {
			...
			if (!command) {
				...
				try {
					...
					} else if (action == "toString" || action == "str") {
...
void basicTestFibonacci() {
    cout << "================== BEGIN: Basic Structural Tests==================" << endl;
    try {
        {
            logInfo("These tests will check size() isEmpty() without calling dequeueMin().");

            FibonacciHeapPriorityQueue queue;
            checkCondition(queue.isEmpty(), "New priority queue should be empty.");
            checkCondition(queue.size() == 0, "New priority queue should have size 0.");
...
void ascendingTestFibonacci() {
    ...
    try {
            ...
            Vector<int> vec;
            for (int i = 0; i < 10000; i++) {
                vec.add(randomInteger(0, 1000000));
            }
            ...
            logInfo("Enqueuing 10000 random strings.");
            foreach (int value in vec)
                queue.enqueue(integerToString(value), value);

            bool correct = true;
            logInfo("Dequeuing 10000 random strings.");
            for (int i = 0; i < vec.size(); i++) {
                string val = queue.dequeueMin();
                string str = integerToString(vec[i]);
                if (val != str) {
                    correct = false;
                    cout << "Returned wrong value:" << endl;
                    cout << "Expected: " << val << "; Returned: " << str << endl;
                    getLine("Press ENTER to continue...");
                }
            }

            checkCondition(correct, "Queue correctly sorted 10000 random strings.");
    } catch (ErrorException& e) {
        cout << "TEST FAILURE: Unexpected exception: " << e.getMessage() << endl;
    } catch (exception& e) {
        cout << "TEST FAILURE: Unexpected exception: " << e.what() << endl;
...
void descendingTestFibonacci() {
    ...
    try {
            ...
            for (int i = 0; i < 10000; i++) {
                randomValues += randomInteger(0, 1000000);
            }

            logInfo("Sorting 10000 random strings.");
            sort(randomValues.begin(), randomValues.end(), greater<int>());
            ...
            for (int i = 0; i < randomValues.size(); i++) {
                string val = queue.dequeueMin();
                string str = integerToString(randomValues[i]);
                if (val != str) {
                    isCorrect = false;
                    cout << "Returned wrong value:" << endl;
                    cout << "Expected: " << val << "; Returned: " << str << endl;
...
void sortRandomTestFibonacci() {
    ...
    try {
            ...
            reverse(randomValues.begin(), randomValues.end());
            for (int i = 0; i < randomValues.size(); i++) {
                if (queue.dequeueMin() != integerToString(randomValues[i])) {
                    isCorrect = false;
                    break;
                }
            }
...
void reuseTestFibonacci() {
    ...
    try {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            Vector<int> ints;
            for (int i = 0; i < 10000; i++) {
                ints += randomInteger(0, 1000000);
            }

            /* Enqueue them into the priority queue. */
            logInfo("Enqueuing 10000 random strings.");
            foreach (int i in ints) {
                queue.enqueue(integerToString(i), i);
            }
            ...
            logInfo("Dequeuing 10000 random strings.");
            foreach(int i in ints) {
                if (queue.dequeueMin() != integerToString(i)) {
                    isSorted = false;
                }
            }
                ...
                try {
                    if (action == "peek" || action == "p") {
                        ...
                        if (!getline(command, toEnqueue)) {
                            ...
                            queue.enqueue(toEnqueue, key);
                            cout << "Enqueued string \"" << toEnqueue << "\"." << endl;
                        }
                    } else if (action == "toString" || action == "str") {
                            cout << queue.toString() << endl;
                    } else if (action == "decreaseKey" || action == "k") {
                        string value = getLine("Enter node's value: ");
                        int key = getInteger("Enter node's key: ");
                        int newKey = getInteger("Enter new key: ");
                        queue.decreaseKey(value, key, newKey);
                    } else {
                        cout << "Unknown command." << endl;
                    }
...
void testFibonacci() {
    basicTestFibonacci();
    ascendingTestFibonacci();
    descendingTestFibonacci();
    sortRandomTestFibonacci();
    reuseTestFibonacci();
}

void testEveryQueue(){
    getLine("\n============== TEST: VectorPriorityQueue ==============\n"
            "Testing VectorPriorityQueue\nPress ENTER to continue...");
    testPriorityQueue<VectorPriorityQueue> ();
    getLine("\n============ TEST: LinkedListPriorityQueue ============\n"
            "Testing LinkedListPriorityQueue\nPress ENTER to continue...");
    testPriorityQueue<LinkedListPriorityQueue> ();
    getLine("\n========= TEST: DoublyLinkedListPriorityQueue =========\n"
            "Testing DoublyLinkedListPriorityQueue\nPress ENTER to continue...");
    testPriorityQueue<DoublyLinkedListPriorityQueue> ();
    getLine("\n=============== TEST: HeapPriorityQueue ===============\n"
            "Testing HeapPriorityQueue\nPress ENTER to continue...");
    testPriorityQueue<HeapPriorityQueue> ();
    getLine("\n=========== TEST: BinomialHeapPriorityQueue ===========\n"
            "Testing BinomialHeapPriorityQueue\nPress ENTER to continue...");
    testPriorityQueue<BinomialHeapPriorityQueue> ();
    getLine("\n=========== TEST: FibonacciHeapPriorityQueue ==========\n"
            "Testing BinomialHeapPriorityQueue\nPress ENTER to continue...");
    testFibonacci();
}
...
void displayMenu() {
	...
	cout << REPL_BINOMIAL_HEAP << ": Manually test BinomialHeapPriorityQueue" << endl;
	cout << TEST_BINOMIAL_HEAP << ": Automatically test BinomialHeapPriorityQueue" << endl;
    cout << REPL_FIBONACCI_HEAP << ": Manually test FibonacciHeapPriorityQueue" << endl;
    cout << TEST_FIBONACCI_HEAP << ": Automatically test FibonacciHeapPriorityQueue" << endl;
    cout << TEST_ALL << ": Automatically test ALL priority queues" << endl;
	cout << QUIT << ": Quit" << endl;
}
...
int main() {
	while (true) {
		...
		case REPL_HEAP:
			...
			break;			
		case TEST_BINOMIAL_HEAP:
			testPriorityQueue<BinomialHeapPriorityQueue> ();
			break;
		case REPL_BINOMIAL_HEAP:
			replTestPriorityQueue<BinomialHeapPriorityQueue> ();
            break;
        case TEST_FIBONACCI_HEAP:
            testFibonacci();
            break;
        case REPL_FIBONACCI_HEAP:
            replFibonacci();
			break;
        case TEST_ALL:
            testEveryQueue();
